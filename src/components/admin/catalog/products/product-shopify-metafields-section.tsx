...
function getAttributeLibraryHref({
  attributeKey,
  attributeId,
}: {
  attributeKey: string;
  ...
  const cleanAttributeKey = String(attributeKey || "").trim();
  const cleanAttributeId = String(attributeId || "").trim();

  if (cleanAttributeKey) {
    params.set("search", cleanAttributeKey);
  }

  if (cleanAttributeId) {
    params.set("attributeId", cleanAttributeId);
  }

  const query = params.toString();

  return `/admin/catalog/attributes/library${query ? `?${query}` : ""}`;
}
...
const selectedTitle = selectedPage
  ...
  ? createPortal(
      ...
      >
        <div className="flex max-h-[86vh] w-full max-w-2xl flex-col overflow-hidden rounded-[1.5rem] bg-white shadow-2xl ring-1 ring-black/10">
          <div className="flex shrink-0 items-start justify-between gap-4 border-b border-neutral-200 px-5 py-4">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs text-neutral-500">
                Current product metafield ke liye CMS page select karo.
              </p>
            </div>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-4">
            ...
            {!pickerError && !isLoading && items.length === 0 ? (
              <div className="flex min-h-[240px] items-center justify-center rounded-xl border border-dashed border-neutral-200 text-sm text-neutral-500">
                No pages found.
              </div>
            ) : null}
                  ...
                  {items.map((page) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="shrink-0 rounded-full bg-neutral-100 px-2.5 py-1 text-[10px] font-semibold text-neutral-600">
                          {page.status || "PAGE"}
                        </span>
                      </button>
                    );
          ...
          <div className="flex shrink-0 items-center justify-between gap-3 border-t border-neutral-200 bg-white px-5 py-4">
            <button
              type="button"
              onClick={() => setIsAddPageOpen(true)}
              className="inline-flex h-9 items-center gap-2 rounded-xl border border-neutral-200 bg-white px-3 text-sm font-semibold text-neutral-800 transition hover:bg-neutral-50"
            >
...
function AttributeLibraryManageLink({
  attributeKey,
  attributeId,
}: {
  attributeKey: string;
  ...
  const href = getAttributeLibraryHref({
    attributeKey,
    attributeId,
  });
